...
(() => {
  ...
  // src/components/bs5/accordion/accordion.functions.js
  function accordionToggleAll(event) {
    try {
    ...
    if (toggleAllButton.classList.contains("accordion-toggle-btn--closed")) {
      ...
      accordionButtons.forEach((button) => {
        if (button.matches(":not(.collapsed)"))
          button.click();
      });
    }
  }
  function accordionToggleAllButtonState(event) {
    try {
      window.event.cancelBubble = true;
      event.stopPropagation();
    } catch (error) {
    }
    let accordionGroup = event.target.closest(".accordion-group"), toggleAllButton = accordionGroup.querySelector(".accordion-toggle-btn");
    if (toggleAllButton.classList.contains("accordion-toggle-btn--closed") && accordionGroup.querySelectorAll(".accordion-button.collapsed").length == 0) {
      toggleAllButton.classList.remove("accordion-toggle-btn--closed");
      toggleAllButton.classList.add("accordion-toggle-btn--open");
      toggleAllButton.textContent = "Close all";
    } else if (toggleAllButton.classList.contains("accordion-toggle-btn--open") && accordionGroup.querySelectorAll(".accordion-button.collapsed").length > 0) {
      toggleAllButton.classList.remove("accordion-toggle-btn--open");
      toggleAllButton.classList.add("accordion-toggle-btn--closed");
      toggleAllButton.textContent = "Open all";
    }
  }
  function accordionHashLinks(event) {
    let urlHash = window.location.hash, urlHashId = urlHash.substring(1), cleanHash = filterSpecialChar(urlHash);
    if (cleanHash.length > 0) {
      let targetElement = document.querySelector(`#collapse-${urlHashId}`), targetPanelButton = false;
      if (event && (event.type === "hashchange" && event.newURL === event.oldURL || event.type === "click" && event.target.hash === urlHash)) {
      ...
      if (targetPanelButton) {
        if (targetPanelButton.matches(".collapsed"))
          targetPanelButton.click();
        window.scrollTo(0, targetPanelButton.getBoundingClientRect().top + document.documentElement.scrollTop);
      }
    }
  }
  ...
  // src/components/bs5/video/video.functions.js
  function videoEmbedPlay(event) {
    event.preventDefault();
    const thumbnail = event.target, component = thumbnail.closest(".video"), iframe = component.querySelector(".video-embed iframe");
  ...
  // src/components/bs5/navbar/navbar.functions.js
  var import_core = __toESM(require_popper(), 1);
  function initializeDropdownPopper(referenceElement, popperElement) {
    const popperInstance = (0, import_core.createPopper)(referenceElement, popperElement, {
  ...
  // src/components/bs5/quickexit/quickexit.functions.js
  ...
  // src/components/bs5/header/header.functions.js
